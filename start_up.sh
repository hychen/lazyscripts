...
case "$DISTRIB_ID" in
    ...
    cat distrib/start_debian_ubunut.sh >> $ENV_EXPORT_SCRIPT
    ;;
    "Debian")
    export ARCH_NAME="`uname -a | cut -d " " -f 12`"
    echo "export ARCH_NAME=\"`uname -a | cut -d " " -f 12`\"" >> $ENV_EXPORT_SCRIPT
    cat distrib/start_debian_ubuntu.sh >> $ENV_EXPORT_SCRIPT 
    ;;
    "SUSE LINUX")
    export ARCH_NAME="`uname -i`"
    echo "export ARCH_NAME=\"`uname -i`\"" 
    cat distrib/start_opensuse.sh >> $ENV_EXPORT_SCRIPT
    ;;
    *)
    echo "Sorry,Lazyscripts not supoort $DISTRIB_ID"
    ;;
esac
